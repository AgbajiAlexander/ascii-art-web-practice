...
# Test with name
RESP_NAME=$(curl -s "$SERVER_URL/hello?name=Alex")
if [[ "$RESP_NAME" == *"Hello, Alex!"* ]]; then
    echo -e "${GREEN}✔ PASS: Query param parsed successfully ('Hello, Alex!')${NC}"
else
    echo -e "${RED}✘ FAIL: Expected 'Hello, Alex!', got '$RESP_NAME'${NC}"
fi
